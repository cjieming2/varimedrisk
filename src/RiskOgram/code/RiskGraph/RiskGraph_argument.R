...
# usage: Rscript RiskGraph.R host db user password p_snps
er password 
# Alex Morgan, Rong Chen, 09/05/2012

db.host <- commandArgs(TRUE)[1]
db.name <- commandArgs(TRUE)[2]
db.user <- commandArgs(TRUE)[3]
db.password <- commandArgs(TRUE)[4]
in.snps <- commandArgs(TRUE)[5]

library(RMySQL)
library(gplots)

con <- dbConnect(dbDriver("MySQL"), dbname=db.name, username=db.user, host=db.host, password=db.password);
...
# Get the risk data
riskAlleles <- dbGetQuery(con, "select * from p_snps")
