...
drawOverallLinear<- function(Disease, Prevalence, Cumulative, SNPCount, ratioLimit=1.5, txtScale=50, prevSize=.1, legcex=.5, showOnlyAggregate=F, segments=F, showNumerics=F, segColor="gray50", ...) {
  ...
  while(Row <= NROW(Data)) {
    ...
    if (Data$Prevalence[Row] < Data$Cumulative[Row]) {
      trigp <-  Data$Prevalence[Row] + (prevSize/3)
    } else {
      trigp <-  Data$Prevalence[Row] -(prevSize/3) # prevSize/10 -> prevSize/3
    }
...
# main function to get the risk alleles and summary and pretest probs
```{r}
#### Main function: Look at overall risk ####
## uncomment this part for one individual or testing
## JC loop connect using one example in R
## comment this out if doing multiple individuals
db.host <- "127.0.0.1"
db.name <- "proj_dz_risk_ipop"
db.user <- "chenj"
db.password <- "3VrTh60IlfiHjLATiVkKn8orM"
snps.all.table <- "69_109_spop_SAS_snps_all"
pretest.table <- "user_chenj.pretest_indian_female"

## JC loop connect using all data using R CMD BATCH
# db.host <- commandArgs(TRUE)[1]
# db.name <- commandArgs(TRUE)[2]
# db.user <- commandArgs(TRUE)[3]
# db.password <- commandArgs(TRUE)[4]
# snps.all.table <- commandArgs(TRUE)[5]
# pretest.table <- commandArgs(TRUE)[6]

## >>>> store results in RData
compiled_riskSummary = list()

## print to log file
print(paste("host:", db.host))
print(paste("database:", db.name))
print(paste("user:", db.user))
print(paste("sample snps table:", snps.all.table))
print(paste("pretest probability table:", pretest.table))
...
#### store the snp_all data from db and table
## required cols = study_cnt, LR, broad_phenotype
# riskAlleles <- dbGetQuery(con, "select * from 63_104_spop_EUR_snps_all")
riskAlleles <- dbGetQuery(con, paste("select * from ", snps.all.table, sep = ""))

## JC: remove duplicates due to multiple fxn_type for one gene 
riskAlleles <- unique(riskAlleles %>% select(-fxn_type))
...
## form new table "riskSummary" from info collected above
riskSummary <- mergeViaRowNames(riskSummaryCt, riskSummaryLRprod, "StudyCount", "LR") ## LR == LRprod
...
## >>>> update compiled_riskSummary
compiled_riskSummary = c(compiled_riskSummary, list(riskSummary))
names(compiled_riskSummary) = c(names(compiled_riskSummary), snps.all.table)

#### pretest probability ####
# preTestProbs <- dbGetQuery(con, "select * from var_varimed_preSep2013.pretest_caucasian_female")
preTestProbs <- dbGetQuery(con, paste("select * from ", pretest.table, sep = ""))
...
write.table(riskSummary, paste("RiskSummary_", snps.all.table,".txt", sep = ""), row.names=F, sep="\t", quote=F)
...
pdf(paste("OverallRisk_", snps.all.table,".pdf", sep = ""), height=7, width=7)
...
multRisk <- multRisk %>% filter(Disease %in% c("Myocardial_infarction", "Stroke", "Type_2_diabetes", "Atrial_fibrillation", "Glucose_intolerance", "Hypertension", "HDL_cholesterol_levels", "Obesity", "Hypertriglyceridemia", "Coronary_artery_disease", "Coronary heart disease"))


## JC make riskOgram
## prevSize = numeric; size of arrows
## txtScale = numeric; size of plot
## showNumerics = boolean; show number of SNPs
drawRiskOverallGraph(multRisk$Disease, multRisk$probability, multRisk$PostTest, multRisk$SNPCount, txtScale=25, prevSize=.05, showOnlyAggregate=T, legcex=.65, lwd=3, segments=T, segColor="gray60", showNumerics=T)
...
## Overall risk linear scale
```{r}
# get the risk over all disease
# pdf(paste("OverallRisk_LinearScale_", snps.all.table,".pdf", sep = ""), height=7, width=7)
multRisk <- riskSummary[riskSummary$SNPCount > 1,]
# multRisk <- riskSummary[riskSummary$Disease != "Creutzfeldt-Jakob disease" & riskSummary$Disease != "Crohn's disease",]
multRisk <- multRisk[order(multRisk$PostTest),]
...
drawOverallLinear(multRisk$Disease, multRisk$probability, multRisk$PostTest, multRisk$SNPCount, txtScale=25, prevSize=.05, showOnlyAggregate=T, legcex=.65, lwd=3, segments=T, segColor="gray60", showNumerics=T)
# dev.off()
...
```{r}
## T2D
# pdf(paste("type_2_diabetes_riskOgram", snps.all.table,".pdf", sep = ""), height=10, width=12)
